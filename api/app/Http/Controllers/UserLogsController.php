...
class UserLogsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$data) {
            return response([
                'message' => 'Failed to fetch userLogs.'
            ], 400);
        }
    ...
    public function store(Request $request)
    {
        ...
        $data = UserLogs::create($request->all());

        if (!$data) {
            return response([
                'message' => 'Failed to create userLog.'
            ], 400);
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $data = UserLogs::find($id)->update($request->all());

        if (!$data) {    
            return response([
                'message' => 'Failed to update userLog.'
            ], 400);
        }
